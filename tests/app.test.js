...
describe("App", () => {
    it("returns an error message for a route that is not defined", async() => {
        const response = await request(app)
        .get("/nowhere")
        .set("Accept", "application/json")

        expect(response.text).toContain(`Not found: /nowhere`)
    })
...
describe("path /pastes", () => {
    ...
    describe("POST method", () => {
        ...
        it("returns 400 if reult is missing", async() => {
            const response = await request(app)
            .post("/pastes")
            .set("Accept", "application/json")
